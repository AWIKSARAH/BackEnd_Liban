...
//   }
// };
// async function getEventAfter() {
//   const date = new date()//now
//   let filters={}
//     let query = Event.find({ start_date: { $gt: date } });
//     if (filters.day) {
//       const nextDay = new Date(date.getTime() + 24 * 60 * 60 * 1000);
//       query = query.find({ start_date: { $gte: nextDay } });
//     }
//     if (filters.week) {
//       const nextWeek = new Date(date.getTime() + 7 * 24 * 60 * 60 * 1000);
//       query = query.find({ start_date: { $gte: nextWeek } });
//     }
//     if (filters.month) {
...
//         date.getDate()
//       );
//       query = query.find({ start_date: { $gte: nextMonth } });
//     }
//     if (filters.year) {
//       const nextYear = new Date(date.getFullYear() + 1, 0, 1);
//       query = query.find({ start_date: { $gte: nextYear } });
//     }
//     const events = await query.exec();
//     return events;
//   }
// const event = { add, getAll, getById, edit, Delete, deleteAll, getPrivateEvent,updateConfirmationById ,latestPlace,getEventAfter};
